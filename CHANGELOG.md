...
- **`upload.ts` 支持解压后执行命令列表**：新增 `afterCMD` 配置项，解压完成后按顺序执行远端命令，支持多条命令，任意命令失败则中止并报错，步骤编号动态适配总步数。

### 修复

- **暗色模式下 `type-tag-*` 类型标签文字不可见**：将 `type-tag-*` 系列样式从 `@layer components` 移出，改为原生 CSS，解决 Tailwind 动态类名 purge 导致样式规则未生成的问题。
...
- **补充 104/101 TI=210 文件服务解析**：为 `104ParserClass` 与 `101ParserClass` 新增按 7.6 定义的文件服务报文解析，支持文件目录召唤、目录召唤确认、读文件激活/确认、读文件数据传输/确认、写文件激活/确认、写文件数据传输/确认，并补充目录ID、文件ID、文件名、文件大小、数据段号、后续标志、校验码等字段解析。
- **前端新增 TI=210 文件服务展示信息**：`HexParser` 的 `file_service` 事件卡片新增附加包类型、信息体地址、操作码、目录名、查询时间、文件列表、目录文件列表、分段校验结果等展示内容，便于直接查看文件服务报文细节。

### 优化

- **解析结果区块支持自动展开并记住手动展开状态**：解析完成后会自动展开有数据的区块；用户手动展开/收起后，后续再次解析将保留该交互状态，不再每次重置折叠面板。
...
- **两个页面取消最大限制，直接填满整个页面，让按钮更和谐**
- **扩大了index的文本输入框，可视报文行数更多**
- **优化upload脚本，现在可以指定远程服务器部署路径了**
